package com.packagename.myapp.view.ReCaptcha;
...
public class ReCaptcha extends Component {
	...
	public ReCaptcha(String websiteKey, String secretKey) {
		...
		div.setAttribute("data-callback", "myCallback"); // Note that myCallback
															// must be declared
															// in the global
															// scope.
	...
	private boolean checkResponse(String response) throws IOException {
		...
		String postData = "secret=" + URLEncoder.encode(secretKey, "UTF-8") + "&remoteip=" + URLEncoder.encode(remoteAddr, "UTF-8")
	...
	public void refitCaptcha() {
		getElement().callFunction("refit");
	}

	public void setRemove() {

		getElement().callFunction("remove");
	}
}